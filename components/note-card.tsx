...
export default function NoteCard({ note }: NoteCardProps) {
  return (
    <article className="surface flex min-w-0 flex-col border-t-4 border-t-orange-300">
      <h2 className="text-lg font-semibold leading-relaxed text-slate-900 [overflow-wrap:anywhere]">{note.title}</h2>
      <p className="mt-3 mb-5 flex-1 whitespace-pre-wrap text-sm leading-7 text-slate-600 [overflow-wrap:anywhere]">{note.content || "Just a title. Very mysterious."}</p>
      <time
